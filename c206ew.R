...
st<- raster("spatialdata/stEW206.tif")
R<-1000
...
write.csv(con, "conductanceEW/test206ew.csv")
...
write.csv(con, "conductanceEW/test206ew_bl.csv")
...
cond<- data.frame(read.csv("conductanceEW/test206ew.csv"))
cond_bl<- data.frame(read.csv("conductanceEW/test206ew_bl.csv"))
conductance<-data.frame(cond$disp, cond$Conduct, cond_bl$Conduct)
colnames(conductance)<-c('disp_dist', 'before GCB','after GCB')
write.csv(conductance, "conductanceEW/test206ew_diff.csv")
