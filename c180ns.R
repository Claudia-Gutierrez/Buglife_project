...
st<- raster("spatialdata/st180.tif")
R<-1000
...
write.csv(con, "conductance/test180ns.csv")
...
write.csv(con, "conductance/test180ns_bl.csv")
...
cond<- data.frame(read.csv("conductance/test180ns.csv"))
cond_bl<- data.frame(read.csv("conductance/test180ns_bl.csv"))
conductance<-data.frame(cond$disp, cond$Conduct, cond_bl$Conduct)
colnames(conductance)<-c('disp_dist', 'before GCB','after GCB')
write.csv(conductance, "conductance/test180ns_diff.csv")
