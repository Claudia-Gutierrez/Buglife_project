...
st<- raster("spatialdata/stEW189.tif")
R<-1000
...
write.csv(con, "conductanceEW/test189ew.csv")
...
write.csv(con, "conductanceEW/test189ew_bl.csv")
...
cond<- data.frame(read.csv("conductanceEW/test189ew.csv"))
cond_bl<- data.frame(read.csv("conductanceEW/test189ew_bl.csv"))
conductance<-data.frame(cond$disp, cond$Conduct, cond_bl$Conduct)
colnames(conductance)<-c('disp_dist', 'before GCB','after GCB')
write.csv(conductance, "conductanceEW/test189ew_diff.csv")
