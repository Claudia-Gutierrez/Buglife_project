...
st<- raster("spatialdata/stEW218.tif")
R<-1000
...
write.csv(con, "conductanceEW/test218ew.csv")
...
write.csv(con, "conductanceEW/test218ew_bl.csv")
...
cond<- data.frame(read.csv("conductanceEW/test218ew.csv"))
cond_bl<- data.frame(read.csv("conductanceEW/test218ew_bl.csv"))
conductance<-data.frame(cond$disp, cond$Conduct, cond_bl$Conduct)
colnames(conductance)<-c('disp_dist', 'before GCB','after GCB')
write.csv(conductance, "conductanceEW/test218ew_diff.csv")