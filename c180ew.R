...
hab<- raster("spatialdata/habitat180.tif") #180 grid @ 10m resolution
st<- raster("spatialdata/stEW180.tif")
R<-1000
...
write.csv(con, "conductanceEW/test180ew.csv")



#Raster of AOI including B-line projects
hab<-raster("spatialdata/habitatBL180.tif") #180 grid @ 10m resolution
st<- raster("spatialdata/stEW180.tif")
R<-1000
...
write.csv(con, "conductanceEW/test180ew_bl.csv")
...
cond<- data.frame(read.csv("conductanceEW/test180ew.csv"))
cond_bl<- data.frame(read.csv("conductanceEW/test180ew_bl.csv"))
conductance<-data.frame(cond$disp, cond$Conduct, cond_bl$Conduct)
colnames(conductance)<-c('disp_dist', 'before GCB','after GCB')
write.csv(conductance, "conductanceEW/test180ew_diff.csv")
